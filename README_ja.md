⚠️ この翻訳は README の大規模な再構成より前のものです — 同期されるまで英語版 [README.md](README.md) を参照してください。 *(This translation predates a major README restructuring — see the English README.md until parity lands.)*
...
sage-wikiには18のMCPツールがありますが、エージェントのコンテキストにWikiを*いつ*チェックすべきかを示すものがなければ、ツールは使用されません。スキルファイルはそのギャップを埋めます — エージェントに検索のタイミング、キャプチャすべき内容、効果的なクエリ方法を教える生成されたスニペットです。
...
- **MCP：** stdioまたはSSE経由の18ツール（7読み取り、9書き込み、2複合）。オンデマンドコンパイル用の`wiki_compile_topic`と知識抽出用の`wiki_capture`を含みます。
