...
- **エージェントと人間のために。** 18のMCPツールと生成されたスキルファイルが、いつ検索し、キャプチャし、コンパイルすべきかをエージェントに教えます。人間には、同じデータの上でObsidianネイティブなmarkdown、TUI、Web UIが提供されます。
...
サーバーは18のツールを公開します — 検索、読み取り、グラフクエリ、キャプチャ、
...
現在の評価（[eval/REPORT.md](eval/REPORT.md)、2026年4月）：合成ベンチマーク
スイートにおいて、全体品質スコア**85.9–86.7%**（検索、抽出、引用、
グラフ整合性の各指標の複合）、検索recall@1 **97.5–99.7%**、recall@10
100%。非LLMコンパイルオーバーヘッド（ハッシュ + 依存関係分析）は
1秒未満に収まります — 実行時間はLLM API呼び出しに支配されます。
[eval/](eval/README.md)のハーネスで再現できます：

```bash
python3 eval/eval.py .               # 自分の Wiki に対する完全な評価
python3 -m unittest discover eval    # ハーネスのセルフテスト
```

## アーキテクチャ

![Sage-Wikiアーキテクチャ](sage-wiki-architecture.png)
...
- **Web UI：** ビルドタグ（`-tags webui`）付きの`go:embed`で埋め込まれたPreact + Tailwind CSS
- **Scribe：** 会話から知識を取り込むための拡張可能なインターフェース。セッションScribeはClaude Code JSONLトランスクリプトを処理します。
- **パック：** 8つのバンドルパック、Gitベースのレジストリ、インストール/適用/削除/更新ライフサイクル、スナップショットロールバック付きトランザクショナル適用、フィルオンリーマージ、設定アローリストによるセキュリティを備えたコントリビューションパックシステム。
- **外部パーサー：** stdin/stdoutサブプロセスプロトコルによるランタイムプラガブルなファイルフォーマットパーサー。タイムアウト、環境変数のストリップ、ネットワーク分離（Linux）によるサンドボックス実行。

CGOゼロ。純粋Go。クロスプラットフォーム。
