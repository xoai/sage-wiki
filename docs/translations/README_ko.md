...
- **컨셉 큐레이션.** 옵트인 패스(`dedup_strategy: "llm"`)가 제안된 컨셉 전체를 한 번에 — 전역 뷰를 가진 유일한 단계에서 — 보고 컨셉마다 keep / fold / drop를 판정합니다. 의미적 재진술은 접히고(별칭과 소스가 병합), 열거형 엔티티는 결상 접히지 않으며(`mw-3`는 `mw-2`가 아님, 유사도와 무관하게), drop은 `llm_dedup.allow_drop`를 켜기 전까지 기록된 제안으로 남습니다.
...
소스 폴더에 파일을 넣기만 하면 sage-wiki가 형식을 자동으로 감지합니다.

**사용자 지정 소스 타입.** `config.yaml`의 소스 루트에 `type:`를 설정하고 `prompts/summarize-{type}.md`를 짝지으면 모든 형식 — PDF와 Office 문서 포함 — 에 고유한 요약 프롬프트와 `source_type` 프론트매터를 부여할 수 있습니다. 이미지는 비전 지원 LLM (Gemini, Claude, GPT-4o)이 필요합니다. 목록에 없는 형식이 필요하신가요? sage-wiki는 [외부 파서](#외부-파서)를 지원합니다 — stdin을 읽고 텍스트를 stdout에 쓰는 모든 언어의 스크립트입니다.
