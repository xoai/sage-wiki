...
- **컨셉 큐레이션.** 옵트인 패스(`dedup_strategy: "llm"`)가 제안된 컨셉 전체를 한 번에 — 전역 뷰를 가진 유일한 단계에서 — 보고 컨셉마다 keep / fold / drop를 판정합니다. 의미적 재진술은 접히고(별칭과 소스가 병합), 열거형 엔티티는 절대 접히지 않으며(`mw-3`는 `mw-2`가 아님, 유사도와 무관하게), drop은 `llm_dedup.allow_drop`를 켜기 전까지 기록된 제안으로 남습니다. 판정 프롬프트는 워크스페이스에서 덮어쓸 수 있습니다(`prompts/curate-concepts.md`).
