...
- **コンセプトキュレーション。** オプトインのパス（`dedup_strategy: "llm"`）が提案コンセプト全体を一度に俯瞰し — グローバルな視点を持つ唯一の段階 — 各コンセプトについて keep / fold / drop を判断します。言い換えは統合され（エイリアスとソースを統合）、列挙型エンティティは決して統合されず（`mw-3` は `mw-2` ではない、類似度がどれだけ高くても）、drop は `llm_dedup.allow_drop` で許可するまで記録された提案のまま留まります。 判定プロンプトはワークスペースで上書きできます（`prompts/curate-concepts.md`）。
...
ファイルをソースフォルダにドロップするだけ — sage-wikiがフォーマットを自動検出します。

**カスタムソースタイプ。** `config.yaml` のソースルートに `type:` を設定し、`prompts/summarize-{type}.md` を対にすれば、どのフォーマット（PDFやOfficeドキュメントを含む）にも独自の要約プロンプトと `source_type` フロントマターを与えられます。（`image` と `code` のソースは内蔵タイプのままです — これらはプロンプトのバリアントではなく、ビジョン／コードパイプラインを選択します。）画像にはビジョン対応のLLM（Gemini、Claude、GPT-4o）が必要です。記載のないフォーマットが必要ですか？sage-wikiは[外部パーサー](#外部パーサー)をサポートしています — stdinを読み取り、テキストをstdoutに書き出す任意の言語のスクリプトです。
