...
- **에이전트와 사람 모두를 위한 설계.** 18개의 MCP 도구와 생성된 스킬 파일이 에이전트에게 언제 검색하고, 캡처하고, 컴파일할지 가르칩니다. 사람은 같은 데이터 위에서 Obsidian 네이티브 마크다운, TUI, 웹 UI를 사용합니다.
...
18개의 도구를 노출합니다 — 검색, 읽기, 그래프 쿼리, 캡처, 온디맨드
...
현재 평가([eval/REPORT.md](eval/REPORT.md), 2026년 4월): 합성 벤치마크
스위트에서 전체 품질 점수 **85.9–86.7%**(검색, 추출, 인용, 그래프
무결성 지표의 종합), 검색 recall@1 **97.5–99.7%**, recall@10
100%. 비 LLM 컴파일 오버헤드(해싱 + 의존성 분석)는 1초 미만이며 —
전체 소요 시간은 LLM API 호출이 지배합니다. 재현은
[eval/](eval/README.md)의 하네스로 할 수 있습니다:

```bash
python3 eval/eval.py .               # 위키에 대한 전체 평가
python3 -m unittest discover eval    # 하네스 자체 테스트
```

## 아키텍처

![Sage-Wiki 아키텍처](sage-wiki-architecture.png)
...
- **팩:** 8개 번들 팩, Git 기반 레지스트리, 설치/적용/제거/업데이트 라이프사이클, 스냅샷 롤백이 포함된 트랜잭션 적용, 채우기 전용 병합, 설정 허용 목록 보안을 갖춘 기여 팩 시스템.
- **외부 파서:** stdin/stdout 서브프로세스 프로토콜을 통한 런타임 플러그형 파일 형식 파서. 타임아웃, 환경 변수 제거, 네트워크 격리(Linux)를 갖춘 샌드박스 실행.

제로 CGO. 순수 Go. 크로스 플랫폼.
